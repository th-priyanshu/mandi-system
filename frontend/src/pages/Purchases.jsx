...
export default function Purchases() {
  ...
  const getPurchases = () => {
    fetch('http://localhost:5001/api/purchases').then(res => res.json()).then(data => setPurchases(data));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch('http://localhost:5001/api/purchases/add', {
      method: 'POST',
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Cyber system se ye khareedi delete karein?")) {
      const res = await fetch(`http://localhost:5001/api/purchases/${id}`, { method: 'DELETE' });
